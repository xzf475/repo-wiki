...
| `indexer/rest_api.py::register_repo` | function | Registers a new repository by parsing request, discovering branches, and creating index |
| `indexer/rest_api.py::task_status` | function | Returns JSON response with task status from request path |
| `indexer/rest_api.py::validate_repo` | function | Validates repository index by checking tracked files, manifests, and stale files |
| `indexer/rest_api.py::sync_repo` | function | Syncs repository by fetching and updating index asynchronously |
| `indexer/rest_api.py::rebuild_repo` | function | Rebuilds repository index from scratch asynchronously |
| `indexer/rest_api.py::_run_all_branches` | function | Acquires per-repo lock and runs a function for all branches |
| `indexer/rest_api.py::sync_all_branches` | function | Syncs all discovered branches via async task |
| `indexer/rest_api.py::reindex_repo` | function | Atomically updates repository metadata, re-discovers branches, and rebuilds all |
| `indexer/rest_api.py::rebuild_all_branches` | function | Rebuilds all discovered branches asynchronously |
| `indexer/rest_api.py::_run_indexing_pipeline` | function | Runs indexing pipeline: parse, enrich, cross-reference, and upsert vectors |
| `indexer/rest_api.py::_run_rebuild_task` | function | Runs rebuild task inner function with per-repo lock acquisition |
| `indexer/rest_api.py::_run_rebuild_task_inner` | function | Performs git fetch, checkout, pull, loads manifest, runs indexing pipeline |
| `indexer/rest_api.py::_run_sync_task` | function | Fetches changes, identifies stale files, updates index |
| `indexer/rest_api.py::_run_register_task` | function | Runs register task inner function with per-repo lock |
| `indexer/rest_api.py::_run_register_task_inner` | function | Stores credentials, fetches repo, loads config, installs webhook |
| `indexer/rest_api.py::unregister_repo` | function | Unregisters repository and removes its index |
| `indexer/rest_api.py::search_symbols` | function | Searches symbols by embedding query, rewriting, expanding with call graph |
| `indexer/rest_api.py::trace_call` | function | Traces call chain for a symbol and returns JSON response |
| `indexer/rest_api.py::get_source_context` | function | Returns source lines around a symbol from file |
| `indexer/rest_api.py::get_edit_context` | function | Returns edit context for symbol across repos |
| `indexer/rest_api.py::resolve_symbol` | function | Resolves symbol to its definition across repos |
| `indexer/rest_api.py::find_tests_for_symbol` | function | Returns test symbols associated with given symbol |
| `indexer/rest_api.py::pre_edit_check` | function | Validates edit impact before applying changes |
| `indexer/rest_api.py::impact_analysis` | function | Analyzes impact of changes on symbol dependencies |
| `indexer/rest_api.py::change_plan` | function | Generates step-by-step change plan for symbol modifications |
| `indexer/rest_api.py::diagnose_index` | function | Diagnoses repository index for missing or stale entries |
| `indexer/rest_api.py::agent_protocol` | function | Returns agent protocol bundle with symbol context |
| `indexer/rest_api.py::locate_from_error` | function | Locates symbol from error message and returns path |
| `indexer/rest_api.py::list_entry_points` | function | Lists entry points for repository sorted by relevance |
| `indexer/rest_api.py::post_edit_verify` | function | Validates diff payload and verifies edit results |
| `indexer/rest_api.py::change_set` | function | Computes change set for symbol modifications from diff |
| `indexer/rest_api.py::coverage_map` | function | Generates test coverage map for repository symbols |
| `indexer/rest_api.py::index_diff_report` | function | Reports differences between current and stale index |
| `indexer/rest_api.py::cross_repo_graph` | function | Builds cross-repository dependency graph for symbol |
| `indexer/rest_api.py::agent_capabilities` | function | Returns agent capabilities manifest as JSON |
| `indexer/rest_api.py::agent_schema` | function | Returns agent schema definition as JSON |
| `indexer/rest_api.py::stable_symbol_id_endpoint` | function | Resolves stable symbol ID from request body |
| `indexer/rest_api.py::index_status` | function | Returns index status for specified repositories |
| `indexer/rest_api.py::list_repos` | function | Lists all registered repositories with details |
| `indexer/rest_api.py::health` | function | Returns health status with number of repos |
| `indexer/rest_api.py::repo_detail` | function | Returns detailed metadata and file listing for a repository |
| `indexer/rest_api.py::wiki_page_content` | function | Returns wiki page content from repository |
| `indexer/rest_api.py::update_repo_meta` | function | Updates repository metadata from request body |
| `indexer/rest_api.py::multi_repo_skill` | function | Executes a skill across multiple repositories and returns results |
| `indexer/rest_api.py::_get_webhook_url` | function | Constructs signed webhook URL for repository |
| `indexer/rest_api.py::_webhook_sign` | function | Signs webhook payload with HMAC-SHA256 and returns hex digest |
| `indexer/rest_api.py::_verify_webhook_sign` | function | Verifies webhook signature using constant-time comparison |
| `indexer/rest_api.py::webhook_by_name` | function | Processes incoming webhook, matches branch rules, and triggers sync |
| `indexer/rest_api.py::create_app` | function | Creates main application with routing, middleware, and static files |
| `indexer/rest_api.py::_index_page` | function | Returns static index HTML page or JSON error |
| `indexer/rest_api.py::_resolve_repos` | function | Resolves repository names from request body items |
| `indexer/rest_api.py::_trace_call_impl` | function | Executes trace call retrieval logic |
| `indexer/rest_api.py::_expand_with_call_graph` | function | Expands search query using call graph retrieval |
| `indexer/rest_api.py::_InvalidBodyError` | class | Custom exception for invalid request body |
| `indexer/rest_api.py::_validate_diff_payload` | function | Validates diff payload size and JSON encoding |
| `indexer/rest_api.py::_parse_body` | function | Parses JSON request body and raises _InvalidBodyError on failure |
| `indexer/rest_api.py::_LoggingMiddleware` | class | ASGI middleware that logs request duration and status |
| `indexer/rest_api.py::_LoggingMiddleware.dispatch` | method | Logs request processing time and calls next middleware |
| `indexer/rest_api.py::_invalid_body_handler` | function | Returns 400 JSONResponse for invalid body errors |
| `indexer/rest_api.py::_AuthMiddleware` | class | ASGI middleware that validates Bearer token against secret |
| `indexer/rest_api.py::_AuthMiddleware.dispatch` | method | Checks Authorization header with Bearer token, returns 401 if invalid |
