...
This test suite validates critical invariants and edge cases across multiple core subsystems: TaskStore, RepoRegistry, RepoLock, manifest loading, config application, and cross-reference merging. TaskStore manages persistent task state (create/update/get/cleanup), RepoRegistry provides thread-safe repo registration, and RepoLock implements skip-lock semantics to avoid deadlocks under concurrent access. Together they support a build orchestration service that must handle concurrent operations, corrupt data, and git failures gracefully. The tests ensure regression coverage for P1 fixes targeting these integration points.

## Modules
| File | Purpose |
|------|---------|
| tests/test_p1_fixes.py | Tests for various P1 bug fixes in task store, registry, manifests, and env |
## Key Symbols
| ID | Type | Description |
|----|------|-------------|
| `tests/test_p1_fixes.py::TestTaskStore` | class | Test suite for TaskStore operations |
| `tests/test_p1_fixes.py::TestTaskStore.test_create_task` | method | Tests creating a task via TaskStore.create |
| `tests/test_p1_fixes.py::TestTaskStore.test_update_task` | method | Tests updating an existing task |
| `tests/test_p1_fixes.py::TestTaskStore.test_update_finished_sets_timestamp` | method | Tests that update sets finished timestamp |
| `tests/test_p1_fixes.py::TestTaskStore.test_update_nonexistent_task_noop` | method | Tests update on nonexistent task does nothing |
| `tests/test_p1_fixes.py::TestTaskStore.test_get_nonexistent_returns_none` | method | Tests get returns None for nonexistent task |
| `tests/test_p1_fixes.py::TestTaskStore.test_cleanup_expired_tasks` | method | Tests cleanup removes expired tasks |
| `tests/test_p1_fixes.py::TestRepoRegistryThreadSafety` | class | Tests thread safety of RepoRegistry register/unregister |
| `tests/test_p1_fixes.py::TestRepoRegistryThreadSafety.test_concurrent_register` | method | Tests concurrent register operations with threads |
| `tests/test_p1_fixes.py::TestRepoRegistryThreadSafety.test_concurrent_unregister` | method | Tests concurrent unregister operations |
| `tests/test_p1_fixes.py::TestRepoLockSkipLock` | class | Tests repo lock skip-lock behavior |
| `tests/test_p1_fixes.py::TestRepoLockSkipLock.test_skip_lock_does_not_release` | method | Tests that skip-lock does not release the lock |
| `tests/test_p1_fixes.py::TestRepoLockSkipLock.test_lock_blocks_concurrent` | method | Tests that lock blocks concurrent access |
| `tests/test_p1_fixes.py::TestRepoRegistryGetNone` | class | Tests get returns None for unregistered repos |
| `tests/test_p1_fixes.py::TestRepoRegistryGetNone.test_get_nonexistent_returns_none` | method | Tests get on nonexistent repo returns None |
| `tests/test_p1_fixes.py::TestRepoRegistryGetNone.test_get_returns_none_safely` | method | Tests get on empty registry returns None |
| `tests/test_p1_fixes.py::TestParseBody` | class | Tests _parse_body function for valid/invalid JSON |
...
| `tests/test_p1_fixes.py::asyncio_coro` | function | Creates async coroutine from awaitable |
| `tests/test_p1_fixes.py::TestManifestFieldValidation` | class | Tests manifest field defaults and corrupt handling |
...
| `tests/test_p1_fixes.py::TestManifestFieldValidation.test_missing_hash_defaults_empty` | method | Tests missing hash defaults to empty string |
| `tests/test_p1_fixes.py::TestManifestFieldValidation.test_corrupt_manifest_returns_empty` | method | Tests corrupt manifest returns empty dict |
| `tests/test_p1_fixes.py::TestApplyEnvEmptyString` | class | Tests that empty string env does not override default |
| `tests/test_p1_fixes.py::TestApplyEnvEmptyString.test_empty_string_does_not_override_default` | method | Tests empty string preserves original config value |
| `tests/test_p1_fixes.py::TestGitReturnCodeCheck` | class | Tests git checkout failure sets task failed |
| `tests/test_p1_fixes.py::TestGitReturnCodeCheck.test_git_checkout_failure_sets_task_failed` | method | Tests git checkout failure marks task as failed |
| `tests/test_p1_fixes.py::TestCrossReferenceMergeCallers` | class | Tests cross_reference merges callers from same and cross-file |
| `tests/test_p1_fixes.py::TestCrossReferenceMergeCallers.test_cross_reference_merges_same_file_and_cross_file_callers` | method | Tests cross_reference merges all callers into one list |
| `tests/test_p1_fixes.py::TestUpdateMetaLock` | class | Tests update_meta uses repo lock |
| `tests/test_p1_fixes.py::TestUpdateMetaLock.test_update_meta_uses_lock` | method | Tests update_meta acquires and releases lock |
| `tests/test_p1_fixes.py::TestEnvQuoteStripping` | class | Tests load_env_file strips double and single quotes |
| `tests/test_p1_fixes.py::TestEnvQuoteStripping.test_double_quotes_stripped` | method | Tests double quotes removed from env values |
| `tests/test_p1_fixes.py::TestEnvQuoteStripping.test_single_quotes_stripped` | method | Tests single quotes removed from env values |
| `tests/test_p1_fixes.py::TestVectorStoreTruncateList` | class | Tests _truncate_list produces valid JSON |
| `tests/test_p1_fixes.py::TestVectorStoreTruncateList.test_truncate_produces_valid_json` | method | Tests _truncate_list output is valid JSON |
| `tests/test_p1_fixes.py::TestVectorStoreTruncateList.test_small_list_not_truncated` | method | Tests small list is not truncated |
| `tests/test_p1_fixes.py::TestGetSourceContextTypeCoercion` | class | Tests string params coerced to int in get_source_context |
| `tests/test_p1_fixes.py::TestGetSourceContextTypeCoercion.test_string_params_converted_to_int` | method | Tests string line numbers converted to int |
| `tests/test_p1_fixes.py::TestUnregisterCleansLock` | class | Tests unregister removes corresponding lock |
| `tests/test_p1_fixes.py::TestUnregisterCleansLock.test_unregister_removes_repo_lock` | method | Tests unregister deletes the repo lock |
| `tests/test_p1_fixes.py::TestAtomicWrites` | class | Tests save_config and save_manifest are atomic |
| `tests/test_p1_fixes.py::TestAtomicWrites.test_save_config_atomic` | method | Tests save_config writes atomically |
| `tests/test_p1_fixes.py::TestAtomicWrites.test_save_manifest_atomic` | method | Tests save_manifest writes atomically |
| `tests/test_p1_fixes.py::TestSingleBranchWhereClause` | class | Tests single-branch repos get WHERE clause |
| `tests/test_p1_fixes.py::TestSingleBranchWhereClause.test_single_branch_gets_where_clause` | method | Tests single branch adds WHERE clause to queries |
| `tests/test_p1_fixes.py::TestIntParamValidation` | class | Tests invalid int params return 400 |
| `tests/test_p1_fixes.py::TestIntParamValidation.test_invalid_line_start_returns_400` | method | Tests invalid line_start returns 400 response |
| `tests/test_p1_fixes.py::TestWebhookBranchCopy` | class | Tests webhook branch list not mutated |
| `tests/test_p1_fixes.py::TestWebhookBranchCopy.test_webhook_branch_list_not_mutated` | method | Tests that branch list is copied before modification |
| `tests/test_p1_fixes.py::TestCleanupSkipsRunning` | class | Tests cleanup does not evict running tasks |
| `tests/test_p1_fixes.py::TestCleanupSkipsRunning.test_cleanup_does_not_evict_running_tasks` | method | Tests cleanup skips tasks in running state |
| `tests/test_p1_fixes.py::TestEmbedQueryEmptyResponse` | class | Tests embed_query raises on empty response |
| `tests/test_p1_fixes.py::TestEmbedQueryEmptyResponse.test_embed_query_raises_on_empty` | method | Tests embed_query raises ValueError for empty result |
| `tests/test_p1_fixes.py::TestComputeHashReturnsNone` | class | Tests compute_hash returns None on OSError |
...
| `tests/test_p1_fixes.py::TestChangedFilesSinceInvalidCommit.test_raises_on_invalid_commit` | method | Tests that invalid commit SHA raises GitCommandError |
| `tests/test_p1_fixes.py::TestExpansionCap` | class | Tests _expand_with_call_graph respects max nodes |
| `tests/test_p1_fixes.py::TestExpansionCap.test_expand_with_call_graph_respects_max` | method | Tests expansion caps at max nodes |
| `tests/test_p1_fixes.py::TestExpansionCapWithExpansion` | class | Tests expansion caps at max with patch |
| `tests/test_p1_fixes.py::TestExpansionCapWithExpansion.test_expand_caps_at_max` | method | Tests expansion stops at maximum nodes |
| `tests/test_p1_fixes.py::TestXSSApiKeyEscape` | class | Tests API key escape prevents XSS |
| `tests/test_p1_fixes.py::TestXSSApiKeyEscape.test_api_key_script_tag_escaped` | method | Tests script tag in API key is escaped |
| `tests/test_p1_fixes.py::TestBearerCaseInsensitive` | class | Tests Bearer token comparison case-insensitive |
| `tests/test_p1_fixes.py::TestBearerCaseInsensitive.test_lowercase_bearer_accepted` | method | Tests lowercase 'bearer' is accepted |
| `tests/test_p1_fixes.py::TestBatchTokenEstimateCJK` | class | Tests CJK characters count triple towards token budget |
| `tests/test_p1_fixes.py::TestBatchTokenEstimateCJK.test_char_budget_uses_triple` | method | Tests CJK characters weighted triple in budget |
| `tests/test_p1_fixes.py::TestRLockNoDeadlock` | class | Tests reentrant lock does not deadlock on register |
| `tests/test_p1_fixes.py::TestRLockNoDeadlock.test_register_repo_no_deadlock` | method | Tests register with RLock does not deadlock |
| `tests/test_p1_fixes.py::TestRLockNoDeadlock.test_rlock_allows_reentrant` | method | Tests RLock allows reentrant acquire |
| `tests/test_p1_fixes.py::TestTopKNegativeValue` | class | Tests top_k negative or zero clamped to one |
...
| `tests/test_p1_fixes.py::TestTopKNegativeValue.test_top_k_normal_value` | method | Tests positive top_k not clamped |
| `tests/test_p1_fixes.py::TestEmbeddingRetry` | class | Tests call_embedding_api has retry logic |
| `tests/test_p1_fixes.py::TestEmbeddingRetry.test_call_embedding_api_has_retry` | method | Tests embedding API call includes retry decorator |
| `tests/test_p1_fixes.py::TestEmptyChoicesProtection` | class | Tests empty choices protection in responses |
...
## Data Flows
- Client requests task creation → TaskStore.create persists → get retrieves → update sets finished timestamp → cleanup removes expired
- Concurrent threads call RepoRegistry.register/unregister → registry maintains consistent state using synchronization
- HTTP body JSON → _parse_body returns dict on valid, empty dict on invalid/non-dict → downstream handlers use result
- Manifest file read → load_manifest fills missing component_ids (default []), missing hash (default ''), returns {} on corrupt JSON
## Design Constraints
